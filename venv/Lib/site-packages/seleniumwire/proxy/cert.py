import os
from subprocess import PIPE, Popen,STARTUPINFO,STARTF_USESHOWWINDOW,SW_HIDE
import threading
import time
...
def generate(hostname, certdir):
    ...
    with _LOCK:
        ...
        if not os.path.isfile(certpath):
            ...
            p1 = Popen(new_cert_req, stdout=PIPE, stderr=PIPE, startupinfo=st)
            p2 = Popen([openssl, 'x509', '-req', '-days', '3650', '-CA', CACERT, '-CAkey', CAKEY,
                        '-set_serial', epoch, '-out', certpath, '-extfile', extpath], stdin=p1.stdout, stderr=PIPE, startupinfo=st)
            p2.communicate()
            p1.communicate()
