...
class Myspider(scrapy.Spider):
    ...
    def parseJson(self, response):
        ...
        for res in resjson:
            if res['knowledgeType'] == 'VideoKnowledge':
                ...
                yield Request(url=item['siteURL'], meta={'item1': item}, callback=self.parseVedio)
    ...
    def parse(self, response):
        ...
        pattern_html = re.compile(r'(/kng/\S*\.htm[l]?.*?)[\'"].*?>(\w*)<.*?/a>', re.S)
        ...
        onclicks = re.findall(re.compile(r'(id="([\w|-]*)"[^\<|^\>]*)?((onclick="(.*?)")|(href="javascript:void\((.*?)\))).*?>([\w|"]*[·\.\.\.]*)<.*?(span>\(([0-9]+)\))*',re.S), response.text)
    ...
    def parseVedio(self, response):
        item = IlexueVideoItem(response.meta['item1'])
        pattern = re.compile(r'video/(.*?).html', re.S)
