...
class IlexueClient(object):
    ...
    def getSsoRaw(self, username, password):
        data1 = open(os.path.dirname(__file__) + '/js/security.min.js', 'r', encoding='utf8').read()
        data2 = open(os.path.dirname(__file__) + '/js/interface.js', 'r', encoding='utf8').read()
        ctx = execjs.compile(data1 + '\n' + data2)
        return ctx.call('getraw', username, password)
    ...
    def learnCourse(self, courseinfo):
        ...
        if len(ids) > 1:
            ids1 = ids[1]
            course['packageId'] = ids0[0:8] + "-" + ids0[8:12] + "-" + ids0[12:16] + "-" \
                                  + ids0[16:20] + "-" + ids0[20:]
            ...
            course['knowledgeId'] = ids0[0:8] + "-" + ids0[8:12] + "-" + ids0[12:16] + "-" \
                                    + ids0[16:20] + "-" + ids0[20:]
        try:
            requests.get(courseinfo['siteURL'], headers=header,
        ...
        for i in range(1, count):
            ...
            try:
                while result["X-Application-Context"] == '' or (
                    ...
                    print(str(i * 2) + "-knowledgeId:" + course["knowledgeId"] + '-siteURL:' + courseinfo["siteURL"])
                    print(result)
            except Exception as e:
                print(e)
    ...
    def study(self):
        ...
        for course in courses:
            self.learnCourse(courseinfo={'id': course[0], 'siteURL': course[1]})
